...
class PickUp(Movement.Movement):
    ...
    def pick(self, block_name):
        ...
        rospy.logwarn("Path Planner - Pick Up - x: %.2f\ty: %.2f\tz: %.2f", block_orientation_in_quaternion[0]*180/pi, block_orientation_in_quaternion[1]*180/pi, block_orientation_in_quaternion[2]*180/pi)

        # Move 5cm above block
        pose.position.z += 0.3

        # Set End Effector orientation to point downwards using quaternions
        orientation_in_euler = [0,180*pi/180, block_orientation_in_euler[2]]
